...
import static yi.widget.SuggestionsAdapter.getColumnString;
...
import yi.support.v1.utils.ResHelper;
import yi.util.IDHelper;
import yi.util.ReflectUtil;
import yi.util.Reflector;
...
public class SearchView extends LinearLayout implements CollapsibleActionView {
    ...
    private OnViewFadeOutListener mOnViewFadeOutListener;
    ...
    private Runnable mShowImeRunnable = new Runnable() {
        public void run() {
            ...
            if (imm != null) {
                Class[] classTypes = {int.class, ResultReceiver.class};
                Object[] classArgs = {Integer.valueOf(0), null};
                try {
                    Reflector.invokeMethod(imm, "showSoftInputUnchecked", classTypes, classArgs);
                } catch (Exception e) {
                }
            }
        }
    ...
    public interface OnViewFadeOutListener {
    ...
    public SearchView(Context context, AttributeSet attrs) {
        ...
        inflater.inflate(R.layout.cld_search_view, this, true);

        mSearchButton = findViewById(R.id.search_button);
        mQueryTextView = (SearchAutoComplete) findViewById(R.id.search_src_text);
        mQueryTextView.setSearchView(this);
        mQueryTextView.setCompoundDrawablesWithIntrinsicBounds(getResources().getDrawable(R.drawable.cld_ic_search_api_baidu_light), null, null, null);
        ...
        mSearchBackIcon.setOnClickListener(mOnClickListener);
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, IDHelper.getStyleableArrByName(IDHelper.SearchView.self), 0, 0);
        setIconifiedByDefault(a.getBoolean(IDHelper.getStyleableByName(IDHelper.SearchView.iconifiedByDefault), true));
        int maxWidth = a.getDimensionPixelSize(IDHelper.getStyleableByName(IDHelper.SearchView.maxWidth), -1);
        if (maxWidth != -1) {
            setMaxWidth(maxWidth);
        }
        CharSequence queryHint = a.getText(IDHelper.getStyleableByName(IDHelper.SearchView.queryHint));
        if (!TextUtils.isEmpty(queryHint)) {
            setQueryHint(queryHint);
        }
        int imeOptions = a.getInt(IDHelper.getStyleableByName(IDHelper.SearchView.imeOptions), -1);
        if (imeOptions != -1) {
            setImeOptions(imeOptions);
        }
        int inputType = a.getInt(IDHelper.getStyleableByName(IDHelper.SearchView.inputType), -1);
        if (inputType != -1) {
        ...
        a = context.obtainStyledAttributes(attrs, IDHelper.getStyleableArrByName("View"), 0, 0);
        focusable = a.getBoolean(IDHelper.getStyleableByName("View_focusable"), focusable);
        a.recycle();
     ...
     */
    public void setOnViewFadeOutListener(OnViewFadeOutListener listener) {
        mOnViewFadeOutListener = listener;
    }
    
    /**
     * Sets a listener to inform when the user the SearchView has been fadeout
     *
     * @param listener the listener to call when the user closes the SearchView.
     */
    ...
    public void showSearchBackIcon(OnClickListener listener, boolean animation)
    {
        if (animation) {
            // search back icon animation
            Animation mInAnimationFuture = AnimationUtils.loadAnimation(
                    getContext(), R.anim.cld_search_view_back_icon_left_in);
            mInAnimationFuture.setDuration(Animatable.DURATION.VIEW.TRANSLATE);
            mInAnimationFuture.setAnimationListener(new AnimationListener(){
                @Override
                public void onAnimationEnd(Animation animation) {
                    // TODO Auto-generated method stub
                    mSearchBackIcon.setVisibility(VISIBLE);
                }
            ...
            if(mSearchBackIcon.getWidth() == 0){
                mSearchBackIcon.setVisibility(VISIBLE);
            }
            mSearchBackIcon.startAnimation(mInAnimationFuture);

            // search field animation
            ObjectAnimator scaleAnim = ObjectAnimator.ofInt(mSearchPlate,
                    "left", mSearchPlate.getLeft(), mSearchPlate.getLeft() + mSearchBackIcon.getWidth());
            scaleAnim.setDuration(Animatable.DURATION.VIEW.TRANSLATE);
            scaleAnim.setInterpolator(new DecelerateInterpolator());
            scaleAnim.addUpdateListener(new AnimatorUpdateListener() {
                @Override
                public void onAnimationUpdate(ValueAnimator animation) {
                    // TODO Auto-generated method stub
                    mSearchPlate.layout((Integer) animation.getAnimatedValue(),
                            mSearchPlate.getTop(), mSearchPlate.getRight(),
                            mSearchPlate.getBottom());
                }
        ...
        if (listener != null)
            mSearchBackIcon.setOnClickListener(listener);
        else
            mSearchBackIcon.setOnClickListener(mOnClickListener);
    }
    public void hideSearchBackIcon(boolean animation)
    {
        if (animation) {
            Animation mOutAnimationFuture = AnimationUtils.loadAnimation(
                    getContext(), R.anim.cld_search_view_back_icon_left_out);
            mOutAnimationFuture.setDuration(Animatable.DURATION.VIEW.TRANSLATE);
            mSearchBackIcon.startAnimation(mOutAnimationFuture);
            ObjectAnimator scaleAnim1 = ObjectAnimator.ofInt(mSearchPlate,
                    "left", mSearchPlate.getLeft(), 0);
            scaleAnim1.setDuration(Animatable.DURATION.VIEW.TRANSLATE);
            scaleAnim1.start();
            scaleAnim1.setInterpolator(new DecelerateInterpolator());
            scaleAnim1.addUpdateListener(new AnimatorUpdateListener() {
                @Override
                public void onAnimationUpdate(ValueAnimator animation) {
                    // TODO Auto-generated method stub
                    mSearchPlate.layout((Integer) animation.getAnimatedValue(),
            ...
            scaleAnim1.addListener(new Animator.AnimatorListener() {
                ...
                public void onAnimationEnd(Animator animation) {
                    // TODO Auto-generated method stub
                    mSearchBackIcon.setVisibility(GONE);
                }
    ...
    private Runnable mShowIMM = new Runnable() {
        public void run() {
    ...
    private void setShowSoftInputOnFocus(boolean show){
        ...
        if(sdkversion < 16){
            try {
                ...
                setShowSoftInputOnFocus.invoke(mQueryTextView, show);
            } catch (Exception e) {
            }
        }else{
            try {
    ...
    public void expandSearchViewUp(OnClickListener listener) {
        if (mQueryTextView != null) {
            setShowSoftInputOnFocus(false);
            showSearchBackIcon(listener, true);
            YiLaf.current().setContentViewExclusive(true);
            postDelayed(mShowIMM, Animatable.DURATION.CONTENT.TRANSLATE);
        }
    }

    private Runnable mHideIMM = new Runnable() {
        public void run() {
            // mQueryTextView.setShowSoftInputOnFocus(true);
            // setImeVisibility(false);
            setShowSoftInputOnFocus(true);
            YiLaf.current().setContentViewExclusive(false);
            hideSearchBackIcon(true);
        }
    };

    public void collapseSearchViewDown() {
        if (mQueryTextView != null) {
            setImeVisibility(false);
            postDelayed(mHideIMM, Animatable.DURATION.CONTENT.TRANSLATE);
        }
    }
    ...
    private void updateViewsVisibility(final boolean collapsed) {
        ...
        mSearchHintIcon.setVisibility(GONE);
        updateCloseButton();
    ...
    private void updateCloseButton() {
        ...
        if(showClose && !mCloseButton.isShown())
        {
	        Animation scale = AnimationUtils.loadAnimation(getContext(), R.anim.cld_search_view_close_button_show);
	        mCloseButton.startAnimation(scale);
        }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        Object actionKey = ReflectUtil.SearchableInfoReflect.findActionKey(mSearchable, keyCode);
        String msg = ReflectUtil.ActionKeyInfoReflect.getQueryActionMsg(actionKey);
        if ((actionKey != null) && (msg != null)) {
            launchQuerySearch(keyCode, msg, mQueryTextView.getText()
                    .toString());
            return true;
        }
        ...
        public boolean onKey(View v, int keyCode, KeyEvent event) {
            ...
            if (!mQueryTextView.isEmpty() && event.hasNoModifiers()) {
                ...
                if (event.getAction() == KeyEvent.ACTION_DOWN) {
                    ...
                    String msg = ReflectUtil.ActionKeyInfoReflect.getQueryActionMsg(actionKey);
                    if ((actionKey != null) && (msg != null)) {
                        launchQuerySearch(keyCode, msg, mQueryTextView
                                .getText().toString());
                        return true;
                    }
                }
    ...
    private boolean onSuggestionsKey(View v, int keyCode, KeyEvent event) {
        ...
        if (event.getAction() == KeyEvent.ACTION_DOWN && event.hasNoModifiers()) {
            ...
            if (keyCode == KeyEvent.KEYCODE_DPAD_LEFT || keyCode == KeyEvent.KEYCODE_DPAD_RIGHT) {
                ...
                ReflectUtil.SearchAutoCompleteReflect.ensureImeVisible(mQueryTextView, true);
            ...
            Object actionKey = ReflectUtil.SearchableInfoReflect.findActionKey(mSearchable, keyCode);
            String msg = ReflectUtil.ActionKeyInfoReflect.getQueryActionMsg(actionKey);
            String msgColumn = ReflectUtil.ActionKeyInfoReflect.getSuggestActionMsgColumn(actionKey);
            if ((actionKey != null)
                    && ((msg != null) || (msgColumn != null))) {
     ...
     *         suggestion
     */
    private static String getActionKeyMessage(Cursor c, Object actionKey) {
        String result = null;
        // check first in the cursor data, for a suggestion-specific message
        final String column = ReflectUtil.ActionKeyInfoReflect.getSuggestActionMsgColumn(actionKey);
        if (column != null) {
        ...
        if (result == null) {
            result = ReflectUtil.ActionKeyInfoReflect.getSuggestActionMsg(actionKey);;
        }
        return result;
    }
    ...
    private int getSearchIconId() {
        ...
        int searchViewSearchIconId = ResHelper.getAttrByName("searchViewSearchIcon");
        getContext().getTheme().resolveAttribute(searchViewSearchIconId,
                outValue, true);
...
/*        // If the field is always expanded, then don't add the search icon to the hint
    ...
    private void updateSearchAutoComplete() {
        ReflectUtil.SearchAutoCompleteReflect.setDropDownAnimationStyle(mQueryTextView, 0);// no animation
    ...
    private void setQuery(CharSequence query) {
        ReflectUtil.SearchAutoCompleteReflect.setText(mQueryTextView, query, true);
    ...
    private void forceSuggestionQuery() {
    	/*try{
			Class listPopWindow = Class.forName("Search");
			Method expandMax = listPopWindow.getDeclaredMethod("setListItemExpandMax", Integer.class);
            expandMax.invoke(mPopup, EXPAND_MAX);
          } 
          catch (Exception ex)
          {
        	  ex.printStackTrace();
    	...
    	try{
			Class autoTextView = Class.forName("AutoCompleteTextView");
			Method doAfterTextChanged = autoTextView.getDeclaredMethod("doAfterTextChanged");
			Method doBeforeTextChanged = autoTextView.getDeclaredMethod("doBeforeTextChanged");
			doAfterTextChanged.invoke(mQueryTextView);
			doBeforeTextChanged.invoke(mQueryTextView);
          } 
          catch (Exception ex)
          {
        	  ex.printStackTrace();
        ...
        public void onWindowFocusChanged(boolean hasWindowFocus) {
            ...
            if (hasWindowFocus && mSearchView.hasFocus() && getVisibility() == VISIBLE) {
                ...
                if (isLandscapeMode(getContext())) {
                    ReflectUtil.SearchAutoCompleteReflect.ensureImeVisible(this, true);
                }
            }
        }
...
/*        void doAfterTextChanged(){
            try{
    			Class autoTextView = Class.forName("AutoCompleteTextView");
    			Method doAfterTextChanged = autoTextView.getDeclaredMethod("doAfterTextChanged", null);
    			doAfterTextChanged.invoke(mSearchView);
              } 
              catch (Exception ex)
              {
            	  ex.printStackTrace();
    ...
    public void isViewFadeOut(){
        if(mOnViewFadeOutListener != null)
            mOnViewFadeOutListener.onViewFadeOut();
    }
}
